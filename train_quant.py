...
from qqquantize.toolbox.fxquantize import fx_adjust_bits_IP1, fx_adjust_bits_IP2
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    mapping[Concat] = qqquantize.qmodules.QConcat
    ...
    # assert len(pg0) == 0
    if opt.adam:
        optimizer = optim.Adam(pg1, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999), weight_decay=hyp['weight_decay'])  # adjust beta1 to momentum
    else:
    ...
    # Image sizes
    gs = int(max(model.module.stride))  # grid size (max stride)
    ...
    # _N = 2000
    # for i, (imgs, targets, paths, _) in tqdm(enumerate(dataloader), total=_N):
    ...
    results, maps, times = test.test(opt.data,
                                     ...
                                     log_imgs=opt.log_imgs if wandb else 0)
    enable_observer(model)
    enable_calc_qparams(model)
    # fake_in = torch.zeros(1, 3, 416, 416).to(device)
    # fx_adjust_bits(model, fake_in)
            ...
            if save:
                with open(results_file, 'r') as f:  # create checkpoint
                    ckpt = {'epoch': epoch,
                            ...
                            'model': ema.ema.state_dict() if ema else model.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyp', type=str, default='data/hyp.quant.yaml', help='hyperparameters path')
